...
def SA(rate):
    ...
    for membrane in m.membranes_state:
        with open('./' + membrane + "_" + rate + '+' + str(changes_rates[rate]) + '.txt', 'wb') as lipid_file:
            line = ""
    ...
    for membrane in m.comp_ratio_dict:
        with open('./' + membrane + "_" + rate + '+' + str(changes_rates[rate]) + '_comp.txt', 'wb') as comp_file:
            line = ""
    ...
    # FA distribution
    with open('./fa_distribution_' + rate + '+' + str(changes_rates[rate]) + '.txt', 'wb') as fa_file:
        line = ""
    ...
    for i in range(3):
        ...
        for membrane in mem:
            with open('./' + membrane + "_" + rate + '+' + str(changes_rates[rate]) + '.txt', 'a') as lipid_file:
                line = ""
        ...
        for membrane in m.comp_ratio_dict:
            with open('./' + membrane + "_" + rate + '+' + str(changes_rates[rate]) + '_comp.txt', 'a') as comp_file:
                line = ""
        ...
        with open('./fa_distribution_' + rate + '+' + str(changes_rates[rate]) + '.txt', 'a') as fa_file:
            line = ""
            for fa in m.saturation_composition_total:
